...
| `is_limit_up(k)` | k=bar下标 | 近似涨停：收于当日最高且较昨收 ≈+10%（主板近似，非精确按板块） |
| `is_limit_down(k)` | k=bar下标 | 近似跌停：收于当日最低且较昨收 ≈-10% |
| `tf(expr, "week"\|"month")` | expr=表达式, period=周期 | 多周期：把日线重采样为周/月线，在对应的周/月 bar 上求值 expr。例：`tf(cross(macddiff(i), macddea(i)), "week")` = 周线 MACD 金叉 |

> `tf` 说明：`i` 在 expr 内部指的是**重采样后**（周/月）的 bar 下标；重采样自当前日线数据，无需额外数据源。月线按 `YYYY-MM` 精确分组，周线按周一对齐。
>
> **暂不支持**：相对大盘强弱（`vs_index`）需要指数行情作为第二数据序列，当前沙箱只接收单只标的的 K 线，故未提供；如需可在数据层引入指数序列后扩展。
