...
function RuleList({ onViewCode }: { onViewCode: (r: TradingRule) => void }) {
  ...
  const deleteRule = useCallback((id: string) => {
    ...
    const updated = rules.filter(r => r.id !== id).map((r, i) => ({ ...r, markerIndex: i + 1, color: ruleColor(i + 1) }));
    setRules(updated); saveRules(updated);
  }, [rules]);

  const resetDefaults = useCallback(() => { setRules(RULE_TEMPLATES); saveRules(RULE_TEMPLATES); }, []);

  const addRule = useCallback((rule: TradingRule) => {
    const maxIdx = rules.reduce((max, r) => Math.max(max, r.markerIndex ?? 0), 0);
    const newRule = { ...rule, id: 'r_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6), enabled: true, markerIndex: maxIdx + 1, color: ruleColor(maxIdx + 1), createdAt: new Date().toISOString() };
    const updated = [...rules, newRule];
    setRules(updated); saveRules(updated);
  }, [rules]);

  const addTemplate = useCallback((tpl: TradingRule) => {
    const maxIdx = rules.reduce((max, r) => Math.max(max, r.markerIndex ?? 0), 0);
    const newRule = { ...tpl, id: 'tpl_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6), enabled: true, markerIndex: maxIdx + 1, color: ruleColor(maxIdx + 1), createdAt: new Date().toISOString() };
    const updated = [...rules, newRule];
    setRules(updated); saveRules(updated);
  }, [rules]);
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[11px]" style={{ color: 'hsl(var(--text-tertiary))' }}>
          {rules.filter(r => r.enabled).length}/{rules.length} 已启用 · K线上显示买卖标记
        </span>
        <button onClick={resetDefaults} className="text-[11px] font-medium" style={{ color: 'hsl(var(--text-tertiary))' }}>重置默认</button>
      </div>
      {rules.length === 0 ? (
        <div className="py-8 text-center text-xs" style={{ color: 'hsl(var(--text-tertiary))' }}>暂无规则，请从下方模板添加</div>
      ) : (
        rules.map(rule => (
          <div key={rule.id} className="flex items-center justify-between py-2 px-2 rounded" style={{ background: 'hsl(var(--bg-card))', border: '1px solid hsl(var(--border-subtle))' }}>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button onClick={() => deleteRule(rule.id)}
                className="px-1.5 py-0.5 text-[10px] font-bold rounded hover:opacity-70" style={{ color: 'hsl(var(--text-tertiary))' }} title="删除">
                ✕
              </button>
            </div>
          </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: 'easeOut' }}
      ...
      <div className="flex-1 overflow-auto space-y-6">
        ...
        <InlineAiParsePanel stockId="" onRulesAdded={handleRulesAdded} autoOpen={aiParseAutoOpen} onVisibilityChange={setAiParseAutoOpen} />
            ...
            {RULE_TEMPLATES.map((tpl, i) => (
              <button key={tpl.id} onClick={() => {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] ml-auto shrink-0" style={{ color: 'hsl(var(--text-tertiary))' }}>{tpl.signal === 'buy' ? '买入' : tpl.signal === 'sell' ? '卖出' : '提醒'}</span>
                </div>
