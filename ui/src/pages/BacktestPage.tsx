  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        <Icon size={14} className="text-slate-500 dark:text-slate-500 dark:text-zinc-500" />
        <span className="text-xs text-slate-500 dark:text-slate-500 dark:text-zinc-500 uppercase tracking-wider">{label}</span>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs text-slate-600 dark:text-slate-600 dark:text-zinc-400">{label}</span>
        <span className="text-xs font-mono-nums text-slate-900 dark:text-slate-900 dark:text-white font-medium">{value}</span>
      </div>
      <input
        type="range"
      ...
      <div className="flex items-center justify-between mb-1.5">
        ...
        <div className="flex items-center gap-1">
          <input
            ...
            className="w-20 bg-slate-100 dark:bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-lg px-2 py-1 text-sm text-slate-900 dark:text-slate-900 dark:text-white text-center font-mono-nums focus:outline-none focus:border-violet-500/50"
          />
          <span className="text-xs text-slate-500 dark:text-slate-500 dark:text-zinc-500">%</span>
        </div>
      </div>
    </div>
  );
...
function TradeTable({ trades }: { trades: TradeRecord[] }) {
  ...
  return (
    ...
    >
      ...
      >
        {expanded ? <ChevronDown size={16} className="text-slate-600 dark:text-slate-600 dark:text-zinc-400" /> : <ChevronRight size={16} className="text-slate-600 dark:text-slate-600 dark:text-zinc-400" />}
                ...
                <tbody>
                  {trades.map((trade, i) => (
                    <tr key={trade.index} className={i % 2 === 0 ? 'bg-white/[0.02]' : ''}>
                      <td className="py-2 px-2 text-slate-500 dark:text-slate-500 dark:text-zinc-500 font-mono-nums">{trade.index}</td>
  ...
  if (!code) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-5">
        <BarChart3 size={40} className="text-slate-400 dark:text-zinc-600" />
    ...
    <div className="space-y-5">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-mono-nums text-xl font-bold text-slate-900 dark:text-slate-900 dark:text-white">{stock?.ticker ?? code}</span>
              <span className="text-xs text-slate-500 dark:text-slate-500 dark:text-zinc-500">{stock?.exchange ?? 'SH'}</span>
            </div>
            <div className="text-xs text-slate-500 dark:text-slate-500 dark:text-zinc-500">{stock?.name ?? '—'}</div>
          </div>
        </div>
                ...
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border transition-all duration-200 ${
                    selected
                      ...
                      : 'bg-slate-100 dark:bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-slate-200 dark:border-white/10 hover:bg-white/[0.07] hover:border-white/15'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Icon size={16} className={selected ? 'text-violet-600 dark:text-violet-600 dark:text-violet-400' : 'text-slate-500 dark:text-slate-500 dark:text-zinc-500'} />
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-slate-900 dark:text-slate-900 dark:text-white">参数配置</h2>
              <span className="text-xs text-slate-500 dark:text-slate-500 dark:text-zinc-500">— {STRATEGIES.find(s => s.id === selectedStrategy)?.name}</span>
            </div>
          </div>
            ...
            >
              ...
              <div className="border-t border-slate-100 dark:border-slate-100 dark:border-white/5 pt-5 mb-5">
                <div className="text-xs font-bold text-slate-900 dark:text-slate-900 dark:text-white mb-3">通用参数</div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs text-slate-600 dark:text-slate-600 dark:text-zinc-400">初始资金</span>
                      <span className="text-xs font-mono-nums text-slate-900 dark:text-slate-900 dark:text-white">¥{params.initialCapital.toLocaleString()}</span>
                    </div>
                    <input
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 bg-slate-100 dark:bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-slate-900 dark:text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500/50"
                      />
                      <button
                        onClick={handleSave}
