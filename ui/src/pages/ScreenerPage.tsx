    ...
    for (const stock of batch) {
      ...
      try {
        ...
        if (n >= 4) {
          let shrinkDrop = true;
          for (let i = 0; i < 3; i++) {
            if (closes[n-1-i] >= closes[n-2-i]) { shrinkDrop = false; break; }
            const ma5 = volumes.slice(0, n-i).reduce((a: number, b: number) => a + b, 0) / Math.min(5, n-i);
            if (ma5 > 0 && volumes[n-1-i] / ma5 >= 0.6) { shrinkDrop = false; break; }
          }
      ...
      if (matched.length % 50 === 0) await new Promise(r => setTimeout(r, 0));
    }

    setResults(matched.sort((a, b) => a.close - b.close));
    setRunning(false);
  };
      ...
      <div className="flex-1 flex gap-2 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-2 overflow-hidden">
          ...
          {running && results.length > 0 && (
            <div className="text-data-xs px-1" style={{ color: 'var(--text-tertiary)' }}>
              已扫描 500 只，匹配 {results.length} 只
            </div>
          )}
          ...
          {results.length > 0 && !running && (
            <div className="flex-1 overflow-auto">
              ...
              <div className="text-data-xs py-2 text-center" style={{ color: 'var(--text-tertiary)' }}>
                共 {results.length} 只股票匹配
              </div>
            </div>
          )}
        </div>
